...
if (fs.existsSync(publicDir)) {
  ...
  console.log(`Static files directory not found at: ${publicDir}`);
  console.log('Backend will serve a status page at "/" instead of static files.');

  app.get('/', (req, res) => {
    res.send(`
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 40px; line-height: 1.6; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #1a73e8; margin-bottom: 8px;">Student Consultation API</h1>
        <p style="color: #5f6368; font-size: 1.1em; margin-top: 0;">Backend Server is Running</p>
        <div style="background-color: #e8f0fe; border-radius: 8px; padding: 20px; margin: 24px 0;">
          <p style="margin-top: 0;"><strong>Development Note:</strong></p>
          <p>The frontend UI is configured to run on port <strong>8080</strong> via Nginx or can be started directly using Flutter.</p>
          <a href="http://localhost:8080" style="display: inline-block; background-color: #1a73e8; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: 500; margin-top: 8px;">Open Application UI (Port 8080)</a>
        </div>
        <div style="font-size: 0.9em; color: #70757a; border-top: 1px solid #dadce0; pt: 20px;">
          <p>If the link above doesn't work, make sure you have started the application using:</p>
          <code style="background: #f1f3f4; padding: 4px 8px; border-radius: 4px; display: block; margin: 8px 0;">docker compose up</code>
          <p>For API health check, visit: <a href="/api/health" style="color: #1a73e8;">/api/health</a></p>
        </div>
      </div>
    `);
  });
